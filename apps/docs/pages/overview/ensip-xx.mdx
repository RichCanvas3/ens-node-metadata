...
Nodes can specify a text record with the key name of `schema`, the value of which points to a JSON schema which declares which metadata attributes can be added to the node as text or data records. The value of `schema` MUST be an HTTPS URI. Recommended pattern:

* **Stable URL** (current schema for the type): `https://schemas.agentictrust.io/agent-node.json`
* **Versioned URL** (pinned, immutable): `https://schemas.agentictrust.io/agent-node/v1.0.0/schema.json`

Legacy values MAY use `ipfs://` or `https://`; validators SHOULD accept both. The canonical schema family uses stable + versioned URLs at `https://schemas.agentictrust.io/` as defined by the ontology configuration.

### W3C-aligned semantic records (optional)

To support ontology-based tooling and interoperability with RDF/SKOS ecosystems, nodes MAY use the following text records in addition to (or instead of) `class` and `schema`:

| Key | Meaning |
|-----|--------|
| `sem:type` | Ontology class URI (e.g. `https://ontology.agentictrust.io/AgentNode`). The ontology defines a default schema for each type. |
| `sem:schema` | Optional. Schema URL for validation. If present, use this (e.g. versioned pin `https://schemas.agentictrust.io/agent-node/v1.0.0/schema.json`); otherwise derive from `sem:type`. |
| `sem:schemaVersion` | Optional. Schema version string (e.g. `1.0.0`) for quick inspection; authority remains the URL in `sem:schema`. |
| `label` | Human-readable display name for the node (e.g. "Research Agent"). |

**Resolution rule:** Prefer `sem:schema` if present; otherwise resolve schema from `sem:type` (ontology default). Clients MUST continue to support the legacy `class` and `schema` records. The canonical T-Box and C-Box are published at the registry's `ontology` and `taxonomy` URIs (e.g. `https://ontology.agentictrust.io/`, `https://taxonomy.agentictrust.io/`).
...
* The schema's `$id` field SHOULD be the canonical versioned URL (e.g. `https://schemas.agentictrust.io/agent-node/v1.0.0/schema.json`). Include a `version` field (e.g. `1.0.0`) for quick inspection.
